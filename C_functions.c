...
// 搜索当日生日健在成员
// (头结点，当日日期，提醒信息) 返回->无
void CheckBirth(Tree T, Date date, char** reminder) {
    if (!T) {
        return;
    }
    ...
    if (T->birth->day == date->day && T->birth->month == date->month && T->alive == true) {
        // 当日生日健在成员，将信息加入提醒信息中
        size_t len = strlen(*reminder) + strlen(T->name) + strlen("今日 过生日\n") + 1;
        char* temp = (char*)malloc(len);
        snprintf(temp, len, "%s今日%s过生日\n", *reminder, T->name);
        free(*reminder);
        *reminder = temp;
    }

    CheckBirth(T->child, date, reminder);
    CheckBirth(T->subbro, date, reminder);
}
...
// 提醒当天生日的健在成员(如有，返回提醒信息，没有返回空字符)
// (头结点，当前日期) 返回->提醒信息
char* RemindBirth(Tree T, Date date) {
    char* reminder = strdup("");
    CheckBirth(T, date, &reminder);

    if (strcmp(reminder, "") == 0) {
        // 今日无家族成员过生日
        free(reminder);
        return strdup("今日无家族成员过生日\n");
    }
    else {
        // 返回提醒信息
        return reminder;
